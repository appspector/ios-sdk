...
To check SDK state you can use `+ (BOOL)isRunning` method or a callback: `+ (void)setStateCallback:(SDKStateCallback)callback` which will be called by the SDK on each state change.
States currently supported are: `SDKStateIdle` - SDK has no active or connecting session and `SDKStateRunning` - SDK is in process of establishing or already has established a session.
